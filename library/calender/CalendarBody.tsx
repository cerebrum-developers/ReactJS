...
const TURQUOISE = 'rgba(7, 141, 255, 0.1)';
const ALL_DAY_ROW = 0;
...
function Calendar<T extends GenericEvent>({
  ...
  const currDate = format(new Date(), 'iii') + ' ' + format(new Date(), 'dd')
  const dayColumns = dayList.map((day, counter) => {
    ...
    const highlight = formattedDayandMonth === currDate
    return {
      title: highlight ? <>
        <span style={{
          color: "rgba(7, 141, 255, 1)"
        }}>{formattedDayandMonth}</span>
  ...
  return (
    <div>
      <Table
        ...
        dataSource={getDayHoursEvents(weekDates, getDayEvents, type,startRange,endRange)}
